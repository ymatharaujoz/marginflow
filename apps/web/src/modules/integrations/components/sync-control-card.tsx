  ...
  const statusInfo = (() => {
    ...
    if (canSync) {
      return {
        ...
        subtitle: isMercadoLivre
          ? "A automação está ativa e você ainda pode rodar uma sincronização manual"
          : "Você pode executar uma sincronização agora",
      };
    }
    return {
      icon: <AlertCircle className="h-4 w-4 text-muted-foreground" />,
      title: "Sincronização indisponível",
      subtitle: translateApiMessage(syncStatus.availability.message) || "Aguarde",
    };
  })();
